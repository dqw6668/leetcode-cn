...
public class P799ChampagneTower{
    public static void main(String[] args) {
        Solution solution = new P799ChampagneTower().new Solution();
        // TO TEST
    }
    //leetcode submit region begin(Prohibit modification and deletion)
    class Solution {
        public double champagneTower(int poured, int query_row, int query_glass) {
            // 定义dp[i][j]为流经该坐标杯子的香槟数
            double[][] dp = new double[102][102];
            dp[0][0] = poured;
            for (int i = 1; i <= query_row; i++) {
                for (int j = 0; j <= i; j++) {
                    dp[i][j] = j == 0 ? Math.max(0, dp[i - 1][j] - 1) / 2 : (j == i ? Math.max(0, dp[i - 1][j - 1] - 1) / 2 : Math.max(0, dp[i - 1][j] - 1) / 2 + Math.max(0, dp[i - 1][j - 1] - 1) / 2);
                }
            }
